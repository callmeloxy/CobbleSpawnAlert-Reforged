...
- Fixed the NeoForge `Pokémon > Specific` configuration page so it now matches the Fabric version more closely
- Fixed missing translations in the specific Pokémon configuration UI
- Fixed the missing delete button for specific Pokémon entries
- Fixed the layout and order of the `Text and Sound` section
- Fixed Pokémon suggestions in the configuration UI
- Fixed specific Pokémon alerts being blocked by the classic category and variant filters
- Fixed `alwaysAlert` so it now correctly forces an alert for a configured Pokémon even when it is not part of a rare category or variant
...
- Added a dedicated `pokemon.json` configuration file
- Added a `default` section for global Pokémon alert defaults
- Added a `specific` section for per-Pokémon alert settings
...
- Added in-game configuration support for specific Pokémon alerts
- Added add and remove support for specific Pokémon entries from the config UI
- Added Pokémon search suggestions by French name, English name, short species ID, and full species ID
- Added automatic species ID normalization, for example `pidgey` becomes `cobblemon:pidgey`
...
- Improved config updates so missing configuration keys are now added automatically without overwriting existing user values
- Improved migration from older Pokémon configuration structures to the 2.1.0 format
...
- Improved the NeoForge port validation with successful local server testing
